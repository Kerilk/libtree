...
class TestLibTree < Minitest::Test
  ...
  def test_substitution
    s1 = @m.substitution(rules: { @m.x => @m.a, @m.y => @m.g(@m.b, @m.b) })
    assert( s1.ground? )
    assert_equal( Set[@m.x, @m.y], s1.domain )
    s2 = @m.substitution(rules: { @m.x => @m.y, @m.y => @m.b })
    assert_equal( Set[@m.x, @m.y], s2.domain )
    refute( s2.ground? )
